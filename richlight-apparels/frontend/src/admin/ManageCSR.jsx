...
export default function ManageCSR() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateCSRCard(editingId, { title: form.title.trim(), description: form.description.trim() || '', imageUrl: form.imageUrl || '' });
        setMessage({ type: 'success', text: 'Updated!' });
      } else {
        await addCSRCard({ title: form.title.trim(), description: form.description.trim() || '', imageUrl: form.imageUrl || '' });
  ...
  return (
    <div style={formStyles.container}>
      ...
      <form style={formStyles.form} onSubmit={handleSave}>
        <h3>{editingId ? 'Edit Card' : 'Add CSR Card'}</h3>
