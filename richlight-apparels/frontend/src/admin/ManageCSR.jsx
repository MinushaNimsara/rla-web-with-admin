...
import { getCSRCards, addCSRCard, updateCSRCard, deleteCSRCard } from '../services/firestore';
...
export default function ManageCSR() {
  ...
  const handleSave = async (e) => {
    ...
    try {
      if (editingId) {
        await updateCSRCard(editingId, { title: form.title.trim(), description: form.description.trim() || '', imageUrl: form.imageUrl || '', date: form.date.trim() || '' });
        setMessage({ type: 'success', text: 'Updated!' });
      } else {
        await addCSRCard({ title: form.title.trim(), description: form.description.trim() || '', imageUrl: form.imageUrl || '', date: form.date.trim() || '' });
  ...
  return (
    <div style={formStyles.container}>
      ...
      <form style={formStyles.form} onSubmit={handleSave}>
        <h3 style={formStyles.formHeading}>{editingId ? 'Edit Card' : 'Add CSR Card'}</h3>
      ...
      <h3 style={formStyles.sectionHeading}>Existing cards</h3>
      {loading ? <p>Loading...</p> : items.length === 0 ? <p>No cards yet.</p> : items.map((p) => (
        <div key={p.id} style={formStyles.card}>
          {p.imageUrl && <img src={p.imageUrl} alt="" style={formStyles.cardImg} />}
          <div style={formStyles.cardContent}><strong>{p.title}</strong>{p.date && <span style={{ display: 'block', fontSize: '0.85rem', color: '#b0b0b0', marginTop: '0.25rem' }}>{p.date}</span>}</div>
